...
read_excel_data  <- function(x,sheet = 1){
  read_excel(paste0("data/", x), sheet = sheet)
}
...
```{r}
sheet2_grupo_a <- map_df(planilhas_grupo_a, read_excel_data, sheet = 2)


sheet2_grupo_a |> 
  mutate(num_linhas = 1:nrow(sheet2_grupo_a)) |> 
  filter(str_detect(`1. HISTÓRICO DA PRODUÇÃO NACIONAL`, "Tabela"))
```
