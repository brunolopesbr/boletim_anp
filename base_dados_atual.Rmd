...
for(i in seq_along(lista_guia3_todas)){
  ...
  {
  if (pull(lista_guia3_todas[[i]][[3][,2]) |> is.character() == TRUE) {lista_guia3_todas[[i]][[3]] <- lista_guia3_todas[[i]][[3]] |> 
  mutate_at(2, as.numeric)}
...
for(i in seq_along(lista_guia3_todas)){
  ...
  if( names(lista_guia3_todas[[i]][[4]]) |> 
  str_detect("Nº") |> sum() > 1 ) {lista_guia3_todas[[i]][[4]] <- lista_guia3_todas[[i]][[4]] |> select(!"Nº")}
