...
tabs1 |> bind_rows(tabs2) |> bind_rows(tabs3)
```
...
lista_guia2_todas <- map(seq_along(planilhas), extrair_todas_tabelas, sheet = 2)
```
...
Criar lista 4 sem as três primeiras planilhas
...
```{r}
tabela01_historico_prod_petroleo <- map_df(lista_guia2_todas, `[[`, 1)   |> 
  # reirar coluna mês
  select(!mes) |> 
...
```{r}
tabela02_historico_prod__gas <- map_df(lista_guia2_todas, `[[`, 2)   |> 
  # reirar coluna mês
  select(!mes) |> 
  gather(key = "Período", value = "produto", -1) |> 
  distinct() |> 
...
try(map_df(lista_guia2_restantes, `[[`, 3)  ) 
```
A coluna "mes", criada durante a importação de dados, aqui representa o mês do boletim em que a informação foi criada, e permanece na base de dados.
...
```{r}
parcial_tab4 <-  map_df(lista_guia2_restantes, `[[`, 4)   |> 
  select(!mes) |> 
  gather(key = "Período", value = "produto", -1) |> 
  distinct() |> 
   mutate(Período = my(Período)) |> 
  filter(!is.na(produto)) 
```
E a tabela 3 das três primeiras planilhas

```{r}
parcial2_tab4 <- map_df(lista_guia2_3_primeiras, `[[`, 3)    |> 
  select(!mes) |> 
  gather(key = "Período", value = "produto", -1) |> 
  distinct() |> 
   mutate(Período = my(Período)) |> 
...
```{r}
tabela04_historico_produção_petroleo_gas_natural <- parcial_tab4 |> 
  rbind(parcial2_tab4) |> 
  unique()
```
...
```{r}
tabela16_maiores_instalacoes_petroleo <- map_df(lista_guia4_restantes, `[[`, 6) 
```
...
```{r}
tabela17_maiores_instalacoes_gas <- map_df(lista_guia4_restantes, `[[`, 7)
```
...
```{r}
tabela18_campos_com_maior_queimas <- map_df(lista_guia4_restantes, `[[`, 8)
```
...
```{r}
tabela01_historico_prod_petroleo |> 
  write_csv("csv/tabela01_historico_prod_petroleo", )

tabela02_historico_prod__gas |> 
  write_csv("csv/tabela02_historico_prod__gas")

tabela03_historico_movimentação__gas_natural |> 
  write_csv("csv/tabela03_historico_movimentação__gas_natural")

tabela04_historico_produção_petroleo_gas_natural |> 
  write_csv("csv/tabela04_historico_produção_petroleo_gas_natural.csv")

tabela05_produção_petroleo_gas_natural_por_estado |> 
  write_csv("csv/tabela05_produção_petroleo_gas_natural_por_estado.csv")

tabela06_produção_petroleo_gas_natural_por_bacia |> 
  write_csv("csv/tabela06_produção_petroleo_gas_natural_por_bacia.csv")

tabela07_produção_petroleo_gas_natural_por_operador |> 
  write_csv("csv/tabela07_produção_petroleo_gas_natural_por_operador.csv")

tabela08_produção_petroleo_gas_natural_por_concessionario |> 
  write_csv("csv/tabela08_produção_petroleo_gas_natural_por_concessionario.csv")

tabela09_movimentacao_gas_natural_destinacao |> 
  write_csv("csv/tabela09_MOVIMENTACAO_GAS_NATURAL_POR_DESTINO.csv")

tabela10_distribuicao_producao_campos_pre_sal |> 
  write_csv("csv/tabela10_DISTRIBUICAO_DA_PRODUCAO_CAMPOS_PRE_SAL.csv")

tabela11_30_pocos_maior_prodcao_petroleo |> 
  write_csv("csv/tabela11_maiores_pocos_de_petroleo.csv")

tabela12_30_pocos_maior_producao_gas |> 
  write_csv("csv/tabela12_maiores_pocos_de_gas.csv")

tabela13_30_maiores_pocos_producao_total |> 
  write_csv("csv/tabela13_maiores_pocos_producao_total.csv")

tabela14_maiores_pocos_terrestres_petroleo |> 
  write_csv("csv/tabela14_maiores_pocos_terrestres_petroleo.csv")

tabela15_maiores_pocos_terrestres_gas_natural |> 
  write_csv("csv/tabela15_maiores_pocos_terrestres_gas_natural.csv")

tabela16_maiores_instalacoes_petroleo |> 
  write_csv("csv/tabela16_maiores_instalações.csv")
  
tabela17_maiores_instalacoes_gas |> 
