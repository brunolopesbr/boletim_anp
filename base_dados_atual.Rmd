...
names(base_consolidada[[1]]) |> 
  str_detect("Nº")  |> sum()

base_consolidada[[12]] |> 
  select(!`Nº`)
```
Retirar as colunas "Nº".
...
Para manter a consistência, os nomes das colunas são os da edição 76 (maio/2024) do boletim

```{r}
planilha76 <- c(extrair_todas_tabelas(nume = 76, sheet = 2), extrair_todas_tabelas(nume = 76, sheet = 3), extrair_todas_tabelas(nume = 76, sheet = 4))
```
Retirar colunas "Nº":
```{r}
i <- 7
