...
```
Continua a haver o problema da coluna "Período. Além disso, há uma inconsistência nome da primeira coluna, que pode ser "Período" ou "Mês/Ano". Mudando todas para "Mês/Ano".
...
i <- 1
for(i in seq_along(lista_guia2_restantes)){
 if (pull(lista_guia2_restantes[[i]][[3]][,1]) |> is.character() == TRUE) {lista_guia2_restantes[[i]][[3]][,1] <- pull(lista_guia2_restantes[[i]][[3]][,1])    |> lubridate::my()  }
  i <- 1 + 1
}
...
```{r}
prov <- map_df(lista_guia4_todas, `[[`, 5) 
...
maiores_pocos_terrestres_gas_natural <- bind_rows(prov2, prov1)
...
```{r}
maiores_instalacoes <- map_df(lista_guia4_todas, `[[`, 6)
```
