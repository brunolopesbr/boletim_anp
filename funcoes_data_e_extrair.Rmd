...
x2017_10 <- read_excel(paste0("data/", planilhas[78]), 
    sheet = 2, 
...
x2017_10 <- read_excel(paste0("data/", planilhas[77]), 
    sheet = 2, 
    range = "O8", col_names = FALSE)
x2017_10 <- pull(x2017_10) 
x2017 <- str_remove(x2017_10, "UTC")
class(x2017_10)
...
i <- 1
meu_df <- data.frame("planilha" = NA, "valor" = NA, "classe" = NA)
for(i in seq_along(planilhas)){
  x2017_10 <- read_excel(paste0("data/", planilhas[i]), 
    sheet = 2, 
    range = "O8", col_names = FALSE)
x2017_10 <- pull(x2017_10) 
...
```{r}
 prov <- read_excel(paste0("data/", planilhas[5]), sheet = 3, .name_repair = "unique_quiet")
  ...
  tamanho_ultima_tabela <- (nrow(prov) - linha_inicial[length(linha_inicial)]) - 2
  
  tamanho_tabela[length(tamanho_tabela)] <- tamanho_ultima_tabela 
...
```
```{r}
# read_excel(paste0("data/", planilhas[5]), sheet = 3, .name_repair = "unique_quiet",
 #          skip = limite_tabelas[1,1], n_max = limite_tabelas[1,2])
...
read_excel(paste0("data/", planilhas[5]), sheet = 3, .name_repair = "unique_quiet",
           skip = limite_tabelas[i,1], n_max = limite_tabelas[i,2])  |> 
    print()
}
...
# Função: definir limites das tabelas

```{r}
definir_limites_tabelas <- function(nume, sheet = sheet){
...
```{r}
definir_limites_tabelas(nume = 5, sheet = 3)
```
